#include "all.h"
/* emscheme.c ((elided)) (THIS CAN'T HAPPEN -- claimed code was not used) */
#ifdef __EMSCRIPTEN__

#include <emscripten.h>

Env uschemeplus_globalenv = NULL;

// Evaluate
void EMSCRIPTEN_KEEPALIVE uschemeplus_eval(const char *defs)
{
    XDefstream xdefs = stringxdefs("<web>", defs);
    if (setjmp(errorjmp))
        return;
    readevalprint(xdefs, &uschemeplus_globalenv, ECHOING);
}
