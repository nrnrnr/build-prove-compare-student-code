...
(* \qvfilbreak1.2in                             *)
(*                                              *)
(* Implementation of the prettyprinter          *)
(*                                              *)
(* The prettyprinter is derived from one written by *)
(* Christian Lindig for the C-- project, which in turn *)
(* is based on Wadler's \citeyearparwadler:prettier *)
(* prettyprinter. The definition of [[doc]] simply gives *)
(* the alternatives. The [[BREAK]] indicates that a *)
(* break is permissible, but if the break is not taken, *)
(* the prettyprinter inserts the selected string *)
(* instead.                                     *)
...
and 'a or_auto
  ...
  | B of 'a
(* These value constructors can be awkward to use, so *)
(* I define convenience functions.              *)
(* <definition of [[doc]] and functions>=       *)
val doc    = TEXT
val brk    = BREAK " "
val indent = INDENT
...
(* \qbreak                                      *)
(*                                              *)
(* Interface to a general-purpose prettyprinter *)
(*                                              *)
(* [*] \uasdl generates C code with reasonable  *)
(* indentation and line breaks. Indentation and line *)
(* breaks are chosen by a technology called     *)
(* prettyprinting. The problem has a long history [cite  *)
(* oppen:prettyprinting, hughes:design, wadler:prettier] *)
(* . The code here is based on Christian Lindig's *)
(* adaptation of Wadler's prettyprinter.        *)
(*                                              *)
(* The prettyprinter's central abstraction is the *)
(* document, of type [[doc]]. A document can be made *)
(* from any string. Subdocuments may be concatenated *)
(* ([[^^]]) to form larger documents, and subdocuments *)
(* may also be indented. (Indentation is relative to *)
(* surrounding documents.) And a [[brk]] document marks *)
(* a spot where a line break may be introduced. *)
(*                                              *)
(* Concatenation and indentation are related by these *)
(* laws:                                        *)
(* <boxed values 10>=                           *)
type doc = doc
...
(* Syntax of paths, types, expressions, and so on *)
(*                                              *)
(* A ``module constructor'' names a module. Just like a *)
(* tycon in uML, it's generative. A module constructor *)
(* is generated for each definition of a named module, *)
(* and also for each formal parameter to a module *)
(* function.                                    *)
(*                                              *)
(* ``Module identifier'' is either a modcon or is the *)
(* special identifier [[NAMEDMODTY]] or         *)
(* [[MODTYPLACEHOLDER]], which is attached to components *)
(* in named module types.                       *)
(*                                              *)
...
(* Paths are formed from names using dot notation and *)
(* functor application. Type [[pathex]] is the syntax, *)
(* and [[path]] is the elaborated form.         *)
(* <paths for \mcl>=                            *)
...
(* \qvfilbreak1.8in                             *)
(*                                              *)
(* As promised in the text, an overload declaration is *)
(* processed as if it were a sequence of single-path *)
(* declarations. Each one is done by internal function *)
(* [[next]].                                    *)
(* <definitions of [[basis]] and [[processDef]] for \mcl>= *)
...
(* \qvfilbreak1.8in                             *)
(*                                              *)
(* When a bare name is entered as a definition, the *)
(* interpreter prints a response based on what the name *)
(* stands for. Because module types generally don't fit *)
(* on one line, [[processDef]] uses the prettyprinter *)
(* defined in \crefasdlml.chap (\cpageref       *)
(* asdlml.prettyprinter).                       *)
(* <definitions of [[basis]] and [[processDef]] for \mcl>= *)
...
(* Note the [[eqPrintPrims]] at the end; module [[Int]] *)
(* also exports the equivalence and printing functions. *)
(*                                              *)
(* \qbreak Next are the two sets of array primitives: *)
(* one for [[ArrayCore]] and another for        *)
(* [[UnsafeArray]]. The types of these modules are also *)
(* defined here.                                *)
(* <boxed values 231>=                          *)
...
val initialBasis =
  let val predefinedTypes =
             ...
             , ";                                               "
             , ";  Client code depends on a complete API, including a "
             , ";  description of the abstraction, a module type, and "
             , ";  a behavioral specification of every exported "
             , ";  operation. The example client below depends on the "
             , ";  [[IntArray]] module, whose abstraction is an array of "
             , ";  integers. The abstraction, which I hope is familiar, "
             , ";  can be described using this metaphor: A value of type "
             , ";  [[IntArray.t]] (or just ``an array'') is a sequence "
             , ";  of boxes, each of which holds a value of type  "
             , ";  [[int]].                                     "
             , ";                                               "
             , ";  The [[IntArray]] module implements the [[ARRAY]] "
             , ";  module type, which is part of the initial basis of \\ "
             , ";  mcl and is defined here:[*]                  "
             , ";  <predefined Molecule types, functions, and modules>= "
             , "(module-type ARRAY"
