...
(* Substitution                                 *)
(*                                              *)
(* [*] The substitutions used in uProlog closely *)
(* resemble the ones used in ML type inference (\cref *)
(* ml.chap). A substitution \subsn is a         *)
(* structure-preserving mapping from terms to terms. *)
(* As in \crefml.chap, a substitution is represented as *)
(* an environment. In uProlog, the environment maps *)
(* logical variables to terms. All the substitution *)
(* functions resemble the functions used to substitute *)
(* types for type variables in \crefml.chap.    *)
(* <boxed values 165>=                          *)
type subst = subst
...
(* And a substitution is applied to a constraint by *)
(* [[consubst]].                                *)
(* <boxed values 169>=                          *)
...
(* [*]                                          *)
(*                                              *)
(* Not much parsing code is shared with other   *)
(* interpreters.                                *)
(* <lexical analysis and parsing for \uprolog, providing [[xdefsInMode]]>= *)
...
(* Lexical functions [[upper]], [[symbol]], and *)
(* [[lower]] are renamed to show their meaning as *)
(* uProlog tokens. And utility function [[commas]] *)
(* builds a parser that parses things separated by *)
(* commas.                                      *)
(* <boxed values 181>=                          *)
...
(* type [[cq]] (see chunk [->] in \crefprolog.chap). But *)
...
(* \qvfilbreak3in                               *)
(*                                              *)
(* The next level of uProlog's parser interprets a *)
(* [[concrete]] value according to the mode. [[BRACKET]] *)
(* values and unit tests are interpreted in the same way *)
...
(* To solve multiple goals \ldotsng, I call     *)
(* [[solveMany]] [\ldotsng] \subsn_id \csucc \cfail, *)
(* where \subsn_id is the identity substitution. *)
(* Function [[solveMany]] manages interactions between *)
(* Byrd boxes, composing substitutions as it goes. If *)
(* substitution \subsn' solves goal g_1, we apply \subsn *)
(* ' to the remaining goals g_2, ..., g_n before a *)
(* recursive call to [[solveMany]]. If that recursive *)
(* call fails, we transfer control to the [[resume]] *)
(* continuation that came from solving g_1, which gives *)
(* us a chance to produce a different substitution that *)
(* might solve the whole lot.                   *)
(*                                              *)
...
(* \qtrim2                                      *)
(*                                              *)
(* To show a solution, function [[showAndContinue]] *)
(* applies the solution's substitution to the free *)
(* variables of the query. Then, if the interpreter is *)
(* prompting, [[showAndContinue]] waits for a line of *)
(* input, and if the line begins with a semicolon, *)
(* [[showAndContinue]] returns [[true]], telling the *)
(* interpreter to continue with the next solution. *)
(* Otherwise it returns [[false]]. And if the   *)
(* interpreter isn't prompting, it's running batch mode, *)
(* which produces at most one solution, so      *)
(* [[showAndContinue]] always returns [[false]]. *)
(* <boxed values 106>=                          *)
...
(* other interpreters, which calls [[processDef]] on *)
(* every definition. In uProlog, a ``definition'' is *)
(* either a clause or a query.                  *)
(* <boxed values 105>=                          *)
type basis = basis
...
(* <definitions of [[checkSatisfiedPasses]] and [[checkUnsatisfiablePasses]]>= *)
      ...
      (* \qbreak The [[check-satisfied]] test has an explicit *)
...
(* unit testing, because the unit tests are different. *)
(* Instead of comparing values or types, a unit test can *)
(* check for satisfiability, and when given an explicit *)
(* substitution, a test can check that the substitution *)
