/*
 * \qvfilbreak2.0in All the interpreter code has now
 * been presented, except for the code that gathers all
 * the interfaces into the all.h header file.
...
/*
 * Type, structure, and value definitions for GC roots
 * 
 * The definition of the [[Roots]] data structure relies
 * on types [[Register]], [[Registerlist]], and
 * [[UnitTestlistlist]].
...
/*
 * The small-step versions of these rules are nearly
 * identical, except that above the line, the big-step
 * judgment \nomathbreak\evale ==>\evalr[']v is replaced
 * by the transitive closure of the small step
 * evaluation relation: \nomathbreak\seval[\emptystack]e
 * \STEPSS\qv rho' sigma' \emptystack. The final value v
 * and store sigma' are used below the line, and the
 * final environment rho' is thrown away. (It is a
 * metatheorem of this semantics that rho' is the same
 * as rho.) \ops Small-Step-DefineOldGlobal \twoline xin
 * dom rho \seval[\emptystack]e \STEPSS\qv rho sigma' \
 * emptystack \evaldef\xval(x, e) -->\prhosigma'{rho(x)
 * |->v} \ops Small-Step-DefineNewGlobal \twoline x\not
 * in dom rho\andalsol\notin dom sigma \q\xset(x,e)rho{x
 * |->l}sigma{l|->\unspec}\emptystack \STEPSS \qv rho
 * sigma'\emptystack \evaldef\xval(x, e) -->\prho{x|->l}
 * sigma'
 * 
 * The interpreter
 * 
 * [*]
 * 
 * The evaluator for \uschemeplus, which uses the stack
 * described in the operational semantics, is presented
 * below. The stack itself is a standard data structure,
 * so its implementation is relegated to \appref
 * schemesa, as are functions for debugging, memory
 * management, and parsing control operators. The rest
 * of the interpreter, including the micro-Scheme
 * parser, initial basis, primitives, and so on, is
 * shared with the micro-Scheme interpreter described in
 * \chaprefscheme.
 * 
 * Of all the interpreters in this book, the \
 * uschemeplus interpreter least resembles a real-life
 * interpreter. The stack inspection for [[long-goto]]
 * is good, and using a stack to help evaluate
 * expressions is common, but using a stack to implement
 * [[break]] and [[continue]] is bizarre; sensible
 * implementors use a stack to manage control transfers
 * between procedures, not within a single procedure.
 * 
 * Interfaces and \chaptocsplitinstrumentation
 * 
 * The evaluation stack
 * 
 * The evaluation stack is a sequence of frames, each of
 * type [[Frame]].
 * <type definitions for \uschemeplus>=
 */
 ...
 * Lowering
 * 
 * The lowering transformation defined in \cref
 * schemes.tab.lower (\cpagerefschemes.tab.lower) takes
 * an additional ``lowering context'' parameter that is
 * not shown in the table. That parameter tells the
 * transformation whether an expression appears inside a
 * loop, inside the body of a function, or both.
 * Operators [[break]], [[continue]], and [[return]] are
 * lowered only when they appear in an appropriate
 * context.
 * <type definitions for \uschemeplus>=
 */
 ...
 *  the mutator (the pending unit tests)
 ...
 * is represented by its form, which is typically an
 * expression with a hole in it. \qbreak And if the
 * expression is a function application, then the frame
 * also stores its original syntax. That syntax is used
 * in error messages when, e.g., a program tries to
 * apply a value that is not a function. [*]
 * <structure definitions for \uschemeplus>=
 */
struct Frame {
...
/*
 * To make [[visitenv]] work, I must expose the
 * representation of environments. (In \crefscheme.chap,
 ...
 * 
 * In the early stages of debugging my collector,
 * I found it useful to look for cycles: after every
 * [[cons]], my code would check to see if the newly
 * allocated cons cell participated in a cycle. In case
 * you might also find it helpful to check for cycles,
 * I've kept the code.
 ...
 * 
 * Lots of people are interested in this book because
 * they are interested in interpreters. But the
 * implementation of interpreters isn't the main event;
 * code in \crefimpcore.chap,scheme.chap and elsewhere
 * shows only the parts of the Impcore and Scheme
 * interpreters that are most relevant to the study of
 * programming languages. That code is the tip of an
 * iceberg, and there's a good deal beneath the surface.
 * Much of it is interesting, some is not. The parts
 * that are generic to writing interpreters, not
 * specific to Impcore, can be found here and in \cref
 * cparse.chap.
 * 
 ...
 *  understood, it is still the most complicated part
 *  of the interpreter. The complexity is justified
 ...
 * 
 * All the infrastructure presented in this appendix is
 * reusable. If you use it to build your own
 ...
 * crefpage(cinterps.code-by-concept. A higher-level
 ...
 *  Value             [[Value]]           \Crefpimpcore.Value.int
                                                                             \na
 *  Function          [[Func]],           \Crefpimpcore.type.Func,
 ...
 *  signaling         [[fprint]]
 *  Printers
 *  Error-signaling   [[synerror]],
 *  printers          [[runerror]],       §§[->] &[->] (pages [->] &[->])
                                                 \Crefpsynerror.imp,runerror.imp
 *                    [[othererror]]
 *  Error helpers     [[checkargc]],      §§[->] &[->] (pages [->] &[->])
                                                            \Crefpcheckargc.imp,
                                                               duplicatename.imp
 *                    [[duplicatename]]
 ...
 * 
 * 
 *  The implementation of Impcore (chapters,
 *  appendices, and files)
 * 
 *  --- Where --- What
 *  ━━━━━━━━━━━━━━━━━━━━━━━━━�
         ��━━━━━━━━━━━━━━━━━━━━━━━
 ...
 *                                          common shift
 *                                          functions
 *       \theader\Cref \moduleenv.c         Operations on
 *   app:impcore: code                      function
 *  that is peripheral                      environments
 *    to the ideas and
 *      is specific to
 *          Impcore 50
 ...
 * Names
 * 
 * The [[Name]] is an abstraction built from a
 * string—a sequence of characters. Two names built from
 * equal strings are identical, even if the strings were
 * located in different parts of memory. That means that
 * two names can be compared for equality using a
 * constant-time pointer comparison, which is not
 * possible with strings.
 * 
 * \qbreak Each name is associated with a string it was
 * built from. The string is stored inside the name. \
 * implabelName
 */
 ...
 * [[pushframe]], [[popframe]], and [[clearstack]]
...
/*
 * Function [[pushframe]] pushes a frame in which
 * [[syntax]] is [[NULL]].
 */

/*
 * Function [[topframe]] returns a pointer to the frame
 * on the top of a stack—the youngest, most recently
 * pushed frame—or if the stack is empty, it returns
 * [[NULL]]. A stack [[s]] can safely be mutated by
 * writing through [[topframe(s)]].
 * <function prototypes for \uschemeplus>=
 */
Frame *topframe(Stack s);  // NULL if empty
/*
 * A frame may hold a saved environment, and such frames
 * are pushed by the special function [[pushenv_opt]],
 * which can optimize tail calls (\cref
 * schemes.tail-calls).
 * <function prototypes for \uschemeplus>=
 */
...
/*
 * To determine the value of an option, the interpreter
 * calls [[getoption]]. If the option is not set,
 * [[getoption]] returns [[defaultval]].
 * <function prototypes for \uschemeplus>=
 */
Value getoption(Name name, Env env, Value defaultval);
/*
 * The instrumentation functions are implemented in \
 * apprefschemesa.
 * 
 * \qvspace-0.5
 * 
 ...
 * \qvspace-0.2
 * 
 * The \uschemeplus interpreter is used not only in this
 * chapter but also in \chaprefgc, which focuses on
 * garbage collection. To help debug the garbage
 * collectors, the interpreter frequently calls the
 * [[validate]] function; provided the argument [[v]]
...
/*
 * Next up is function application, which calls for
 * infrastructure that can update a list of expressions
 * within a frame.
 * 
 * Updating lists of expressions within frames
 * 
 * [*]
 * 
 * \xapply and \xlet expressions evaluate expressions in
 * sequence. Values are accumulated by repeating a
 * transition like the one in the \rulename
 * Small-Step-Apply-Next-Arg rule on \cpageref
 * schemes.Small-step-Apply-Next-Arg, which takes a
 * frame like \mathbox\capply(v_f, v_1, ..., v_i-1, \
 * hole, e_i+1, ..., e_n) to one like \mathbox\capply
 * (v_f, v_1, ..., v_i-1, v, \hole, e_i+2, ..., e_n).
 * In the originating frame, the sequence \mathbox v_1,
 * ..., v_i-1, \hole, e_i+1, ..., e_n can be represented
 * as a value of type [[Explist]], where expressions
 * e_i+1...e_n come from the original syntax, and values
 * v_1, ..., v_i-1 are represented as \xliteral
 * expressions. The transition is made by function
 * [[transition_explist]], which overwrites the hole
 * with [[v]], writes a new hole one position to the
 * right, and returns the expression that was
 * overwritten by the new hole. That expression is
 * stored in static memory, so subsequent calls to
 * [[transition_explist]] overwrite previous results.
 * <function prototypes for \uschemeplus>=
 */
Exp transition_explist(Explist es, Value v); // pointer to static memory
/*
 * When the hole is in the rightmost position,
 * [[transition_explist]] overwrites the hole with [[v]]
 * and then returns [[NULL]].
 */

/*
 * What about initializing a frame by putting a hole in
 * the first position? Function
...
/*
 * A function like [[transition_explist]] helps
 * implement \xapply, \xlet, and \xletrec, but it has to
 * be used carefully: if an [[Explist]] is overwritten,
 * it can't be the original [[Explist]] from the
 * syntax—it has to be a copy. The copy should be made
 * when the frame containing the [[Explist]] is first
 * pushed. For example, when the interpreter pushes a
 * frame like \xapply(\hole, \ldotsne), it copies the
 * list of expressions using function [[copyEL]]:
 * {indented} \monopushframe(mkApplyStruct(mkHole(),
 * copyEL(\ldotsne)), evalstack). {indented} Function
 * [[copyEL]] copies a list of expressions, and when the
 * interpreter is finished with the copy, [[freeEL]]
 *  recovers the memory.
 * <function prototypes for \uschemeplus>=
 */
...
/*
 * The interpreter calls [[freeEL]] when popping a frame
 * that contains a copied [[Explist]].
 * 
 * \qbreak When an [[Explist]] appears in an \xapply, \
 * xlet, or \xletrec frame, each element goes through
 * three states:
 * 
 ...
 *  original expression.
 * 
 * Once every element has reached its final state, the
 * [[Explist]] contains only literals, and it can be
...
/*
 * These tools enable us to interpret forms that
 ...
 * 
 * To evaluate an application, push an \apply frame in
 * which the function is replaced with a hole, and start
 * evaluating the function. As explained in the previous
 * section, the list of [[actuals]] in the frame must be
 * a copy of the list in the syntax.
 */
...
/*
 * Extended definitions also have to be lowered.
 * Extended definitions include unit tests, and a unit
 * test can contain control operators, so unit tests
 * have to be lowered. But a unit test can't be lowered
 * until just before it is run—if lowering fails with a
 * run-time error, the error has to occur in the right
 * dynamic context.
 * <function prototypes for \uschemeplus>=
 */
void lowerXdef(XDef); // lower every expression in this definition
/*
 * The mutator makes sure that before any call to
 * [[allocate]], [[roots]] is up to date and contains
 * pointers to all locations that could affect the rest
 * of the computation. The garbage collector inspects
 * the roots and also updates pointers to objects that
 * it moves. Register roots are added and removed in
 * last-in, first-out order using functions [[pushreg]]
 * and [[popreg]].
 * <function prototypes for \uschemeplus>=
 */
...
#endif  /*OMIT*/
/*
 * If the pointer passed to [[popreg]] is not equal to
 * the pointer passed to the matching [[pushreg]], it is
...
/*
 * When a system must support objects of varying sizes,
 * a copying allocator performs well without any
 * additional data structures or adaptations. Unlike a
 * mark-and-sweep allocator, a copying allocator works
 * the same way regardless of how many bytes are
 * requested. It does not need multiple free lists,
 * first-fit search, or any other strategy to find a
 * chunk of free memory of an appropriate size.
 * It simply tests and increments.
 * 
 * Collector overhead. A copying garbage collector does
 * work proportional to the size of the live data. The
 * work itself is more expensive than for the
 * mark-and-sweep system, since the collector must copy
 * objects, not just set mark bits. But copying also
 * produces a benefit: it ``compacts'' the live objects,
 * putting them in adjacent locations in memory.
 * Compaction improves the performance of the machine's
 * cache and virtual memory, but more importantly,
 * it enables fast allocation.
 * 
 * Memory overhead. Our copying system pays only two
 * memory overheads:
 * 
 *   • Because one semispace is always empty, a copying
 *  system requires at least \nomathbreakgamma>=2.
 *  The ``headroom'' needed to get adequate
 *  performance is much larger than in a
 *  mark-and-sweep system. As gamma gets large,
 *  however, this difference between copying and
 *  mark-and-sweep disappears, and the most
 *  significant cost is the cost of allocation.
 * 
 *   • Like the mark-and-sweep collector, the copying
 *  collector must know where to find pointers in
 *  heap objects. The knowledge is implied by the
 *  [[alt]] field.
 * 
 * A copying system has no memory overhead for mark
 * bits. And because our system stores the gray objects
 * on the heap itself, in the style of Cheney \
 * citeyearparcheney:compacting, it requires no stack or
 * work list.
 * 
 * Pause time. Copying allocation always takes constant
 * time; the only pauses are at collections. These
 * pauses take time proportional to the amount of live
 * data.
 * 
 * \qbreak
 * 
 ...
 * [*]
 * 
 * A garbage-collected system can exhibit two kinds of
 * faults:
 * 
 *   • The system can fail to recycle an unreachable
 *  object. Such a fault is a memory leak. A memory
 *  leak does not affect correctness, only
 *  performance: it makes the garbage collector run
 *  more often than it should, and it makes the heap
 *  grow faster than it should. Since we rarely know
 *  how often a collector ``should'' run or how fast
 *  a heap ``should'' grow, these symptoms are hard
 *  to spot—a slow memory leak may go unnoticed for a
 *  long time. A memory leak is easiest to spot when
 *  it makes the heap grow so fast that memory is
 *  exhausted.
 * 
 *   • The system can recycle an object that is still
 *  reachable. When such an object is reused, its
 *  contents change. From the perspective of an old
 *  pointer to the object, the change happens for no
 *  reason, at an unpredictable time. Such a fault,
 *  which could occur if a root is overlooked or if
 *  a tracing procedure fails to follow a pointer,
 *  can be difficult to detect.
 * 
 * Memory leaks aren't catastrophic. You can detect them
 * with a tool like Valgrind or Pin (\secref
 * gc.further-reading), which instruments the code and
 * looks for ``lost'' memory. If you don't have such a
 * tool, you might still find a leak by observing that
 * the garbage collector is retaining too much live
 * data. To know how much is too much, you might rely on
 * regression testing or on careful analysis. Once
 * you're sure there's a leak, you (or a tool) can
 * examine the heap to find out exactly which objects
 * are not being recycled and by what combination of
 * pointers those objects are reachable.
 * 
 * Premature recycling is harder to detect. It usually
 * occurs because the mutator has kept a root (a pointer
 * to a heap object) and hasn't told garbage collector.
 * You might detect such a problem by means of
 * aggressive assertions, also using a tool like
 * Valgrind or Pin—this time to flag reads from or
 * writes to an object that has been reclaimed but not
 * yet reused—or by one of the techniques below.
 * That said, you probably won't have such problems
 * while working the \namecrefsgc.ex.ms-skip, because
 * the code that keeps [[roots]] up to date has been
 * tested extensively.
 * 
 ...
 * [*]
 * 
 * If you need to find memory errors, use my debugging
 * code. As described below, it tracks the three states
 * shown in \figrefgc.memstates.
 * 
 *   • Memory that is owned by the operating system
 *  shouldn't be touched. Neither the collector nor
 *  the mutator should read or write it.
 *   • Memory that is owned by the collector should be
 *  read or written only by collector code.
 *   • Memory that is owned by the mutator may be read
 *  or written freely. The collector should read or
 *  write it only during a call to [[allocloc]].
 * 
 * Memory changes ownership by means of the labeled
 * transitions in \vrefgc.memstates. Memory is acquired
 * in large blocks using [[malloc]]; those blocks are
 * released using [[free]]. A single object is allocated
 * by calling [[allocloc]]; if [[allocloc]] calls the
 * garbage collector, the collector will reclaim
 * unreachable objects. To enable debugging, each of
 * these transitions should be announced by calling the
 * \qbreak debugging functions declared below. These
 * functions mark collector-owned objects as
 * [[INVALID]], and when Valgrind is available, they get
 * it to enforce these rules:
 * 
 *   • No code should read or write any object owned by
 *  the collector. If the collector mistakenly
 *  reclaims a reachable object, then the next time
 *  any code reads or writes that object, Valgrind
 *  will complain.
 *   • A newly allocated object must be initialized
 *  before it is read. If the mutator tries to read a
 *  newly allocated object, perhaps through a stale
 *  pointer, Valgrind will complain.
 * 
 * The debugging functions are implemented in \secref
 * gcsa.gc-debug, and their interfaces are described
 * below.
 * 
 * ━━━━━━━━━━━━━━━━━━━━━━━━━�
�━━━━━━━━━━━━━━━━━━━━━━━━━━�
                                                                              ��
 * 
 *  {tikzpicture}[>=latex',line join=bevel,thick,x=
 *  0.65in,y=0.65in,inner sep=1.8mm] \node(collector)
 *  at (0,0) [draw,rounded rectangle] Owned by heap;
 *  \node(os) at (0,1) [draw,rounded rectangle] Owned
 *  by operating system; \node(mutator) at (0,-1)
 *  [draw,rounded rectangle] Owned by mutator; \draw
 *  [->] (collector) edge [bend left] node [auto]
 *  release (os) ; \draw[->] (os) edge [bend left]
 *  node [auto] acquire (collector); \draw[->]
 *  (collector) edge [bend left] node [auto] allocate
 *  (mutator) ; \draw[->] (mutator) edge [bend left]
 *  node [auto] reclaim (collector); {tikzpicture}
 * 
 * States of memory [*]
 * ━━━━━━━━━━━━━━━━━━━━━━━━━�
�━━━━━━━━━━━━━━━━━━━━━━━━━━�
 ...
 * before any object has been delivered to the mutator
 * via [[allocloc]], the collector should call
 ...
 * is no longer needed and is about to be released, the
 * collector should call [[gc_debug_pre_release]] just
 * before calling [[free]]. As with acquisition, the
 ...
 * the mutator, it should call
 * [[gc_debug_pre_allocate]].
 * <function prototypes for \uschemeplus>=
 */
void gc_debug_pre_allocate(Value *mem); 
/*
 * When the garbage collector decides an object is
 ...
 * object [[INVALID]], and then it will tell Valgrind to
 * complain about any accesses to it. Even without
 * Valgrind, the function can help find bugs; for
 * example, if the last cons cell in \monobox'(a b c) is
 * reclaimed prematurely, you'll see the list turn into
 * \monobox'(a b . <invalid>), making the fault easy to
 * observe.
 * <function prototypes for \uschemeplus>=
 */
void gc_debug_post_reclaim(Value *mem); 
/*
 * To reclaim an entire semispace at once, the copying
 * collector may call function
 * [[gc_debug_post_reclaim_block]]. When the heap grows,
 * the collector must pass the old, smaller size, not
 * the new, larger size.
 * <function prototypes for \uschemeplus>=
 */
void gc_debug_post_reclaim_block(Value *mem, unsigned nvalues); 
/*
 * If the tools above aren't enough, use functions
 * [[gcprint]] and [[gcprintf]] to write whatever you
 * like to standard error. Function [[gcprint]] works
 * like [[print]], and [[gcprintf]] works like
 * [[printf]], but they work only when the environment
 * variable GCVERBOSE is set. If GCVERBOSE is not set,
 * [[gcprint]] and [[gcprintf]] do nothing.
 * <function prototypes for \uschemeplus>=
 */
void gcprint (const char *fmt, ...);  /* print GC debugging info */
void gcprintf(const char *fmt, ...);
/*
 * The debug code is initialized by [[initallocate]] in
 * \apprefgcsa, which calls [[gc_debug_init]].
 * <function prototypes for \uschemeplus>=
 */
...
/*
 * The rest of the interface supports allocation. The
 * managed-heap function [[allocloc]] provides an
 * uninitialized location; in chunk [->], it is used to
 * implement [[allocate]].
 * <function prototypes for \uscheme>=
 */
Value *allocloc(void);
 ...
 */
/*
 * Valgrind gets its information via calls that are
 * described in Valgrind's documentation for ``custom
 * memory allocators.''
 * 
 * When the heap is initialized, my code creates a
 * [[gc_pool]], which stands for all objects allocated
 * using [[allocloc]]. The flag [[gc_uses_mark_bits]],
 * if set, tells Valgrind that when memory is first
 * allocated, its contents are zero. My code also
 * initializes the [[gcverbose]] flag.
 * <global variables used in garbage collection>=
 */
extern bool gc_uses_mark_bits;
/*
 * This [[struct]] is the data structure that is shared
 * between the mutator and the garbage collector, as a
 * global variable:
 * <global variables used in garbage collection>=
 */
extern struct Roots roots;
/*
 * Finally, the maximum size of the stack, in a single
 * evaluation, is tracked in global variable
 * [[high_stack_mark]].
 * <global variables for \uschemeplus>=
 */
extern int high_stack_mark;  // maximum number of frames
/*
 * Except for [[pushenv_opt]], which is described in \
 * secrefpageschemes.optimized-tail-calls,
 * the implementations of the stack functions are
 * relegated to \apprefschemesa.
 * 
 * Instrumentation
 * 
 * [*] To help you understand what happens on the
 * evaluation stack, the \uschemeplus interpreter is
 * instrumented with three options. Each option is a \
 * uschemeplus variable whose value can influence the
 ...
 *  negative, all steps are shown. To change [[ ---
 *  trace-stack]] from a number to a non-number is an
 *  unchecked run-time error.
 * 
 * \stdbreak Options [[ --- optimize-tail-calls]] and [[
 * --- show-high-stack-mark]] are used to set two global
 * variables inside the interpreter:
 * <global variables for \uschemeplus>=
 */
